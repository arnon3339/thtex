...
export type XeLaTeXWorkerResponse =
  | ({ type: "ready" } & XeLaTeXReadyInfo)
  | { type: "initialization-error"; message: string }
  | ({ type: "initialization-status" } & XeLaTeXStatusEvent)
  | ({ type: "status" } & XeLaTeXStatusEvent & { requestId: string })
  | ({ type: "log" } & XeLaTeXLogEvent)
  | {
      type: "success";
      requestId: string;
      pdf: ArrayBuffer;
